...
在这图里，左右分支之所以要有一定的宽度，是为了能够有足够的空间去显示子分支，显然子分支的层数越多，就需要更大的宽度来显示。
...
如果要修改为红黑树，可以自己改节点类，加上颜色属性并修正字符串表示方法。(修改下面代码中TODO的地方即可)
